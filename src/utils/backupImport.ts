...
function filesInFolder(
  ...
  return [...entries.entries()].filter(([path]) => {
    if (!path.startsWith(prefix) || !path.toLowerCase().endsWith(ext))
      return false;
...
async function loadPhotos(
  ...
  const files = [...entries.entries()].filter(
    ([path]) =>
      path.startsWith(PHOTO_FOLDER) &&
      /\.(webp|png|jpe?g|gif|svg|bin)$/i.test(path),
  );
...
function lookupPhoto(photos: PhotoIndex, rawPath: string): string | undefined {
  ...
  return (
    ...
    photos.get(
      cleaned.replace(/\.[^.]+$/, "").slice(cleaned.lastIndexOf("/") + 1),
    )
  );
}
...
function parseAvatarId(value: string | undefined): string | null {
  const raw = value?.trim();
  if (!raw || raw === "—" || raw === "-") return null;
  return raw;
}
...
function hydrateBackupAttachment(
  ...
  const url = item.file
    ? lookupPhoto(photos, item.file)
    : lookupPhoto(photos, item.id);
  if (!url) return null;
  return {
...
function parseSubTasks(
  ...
  const groups: Array<{ completed: boolean; title: string; rest: string[] }> =
    [];
  let current: { completed: boolean; title: string; rest: string[] } | null =
    null;
  ...
  return groups.map((group) => {
    const id = generateId();
    const rest = group.rest.map((line) => line.replace(/^ {2}/, "")).join("\n");
    ...
    return {
      ...
      attachments: images
        .map((image) =>
          attachmentFromImage(image, "subtask", id, photos, createdAt),
        )
...
function parseNotes(
  ...
  return splitByHeading(body, 3).map((section) => {
    ...
    return {
      ...
      attachments: images
        .map((image) =>
          attachmentFromImage(image, "note", id, photos, createdAt),
        )
        .filter((item): item is Attachment => Boolean(item)),
      createdAt,
      updatedAt: createdAt,
...
function parseTaskMarkdown(
  ...
  const preamble = (
    firstH2 === -1 ? markdown : markdown.slice(0, firstH2)
  ).trim();
  const title = preamble.match(/^#\s+(.+)$/m)?.[1]?.trim() || "未命名任務";
  const meta = parseKeyValueTable(preamble);
  const sections = Object.fromEntries(
    splitByHeading(firstH2 === -1 ? "" : markdown.slice(firstH2), 2).map(
  ...
  return {
    ...
    completed: meta["完成"] === "是",
    avatarId: parseAvatarId(meta["頭像"]),
...
function parseLabelsMarkdown(markdown: string): {
  ...
  const sections = Object.fromEntries(
    splitByHeading(markdown, 2).map((section) => [
      section.heading,
      section.body,
    ]),
  );
  ...
  const labelData =
    labelRows[0]?.[0] === "名稱" ? labelRows.slice(1) : labelRows;
  ...
  const statusData =
    statusRows[0]?.[0] === "名稱" ? statusRows.slice(1) : statusRows;
...
function hydrateTask(task: BackupFileTask, photos: PhotoIndex): Task {
  const attachments = asArray<BackupFileAttachment>(task.attachments)
    ...
    .filter((item): item is Attachment => Boolean(item));
  const subtasks = asArray<BackupFileTask["subtasks"][number]>(
    task.subtasks,
  ).map((sub) => ({
    ...sub,
  ...
  const notes = asArray<BackupFileTask["notes"][number]>(task.notes).map(
    (note) => ({
      ...note,
