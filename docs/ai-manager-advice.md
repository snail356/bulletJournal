...
依每日回顧與當日任務，讓 Gemini 以「務實、具體、可執行的 AI 主管」角色產出總評與行動建議。
...
每次呼叫 API 都是獨立的單次請求，不會自動帶入其他日期的日誌、先前對話或跨日記憶。API 的 `systemInstruction` 固定要求模型扮演務實的 AI 主管並使用繁體中文；使用者訊息中的 context 再依序由以下內容組成：
